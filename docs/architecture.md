...
pinned to MCP PID/start-time and client PID/start-time. For recognized remote
server-wrapper topologies, the client may strip the CCC environment before spawning
...
app-server or Claude Remote `ccd-cli` inside the launch boundary for status/read-only
use, but marks the transport non-destructive and grants no workspace authority.
Initial MCP admission retries only the transient client-registration readiness
failure for a bounded interval; identity, ancestry, and authorization failures
remain immediate. Process identity alone is not commit authority: setup
